...
public class BalanceLog {
	public double balancePoint(Log log) {/*
		 if(log instanceof Line){
			 double totalWt = log.weightUpto(log.length());
			 if(log.weightUpto(log.length()/2.0) == totalWt/2.0)){
				 return log.length()/2.0;
			 }
			 else if(log.weightUpto(log.length()/2.0) > totalWt/2.0){
				 for()
			 }
			 return ()
		 }else if(log instanceof Exp){
			 
		 }else if(log instanceof Power){
			 
		 }else if(log instanceof Sine){
