	...
	public static void main(String[] args) {
		...
		while (!input.equals("exit")) {
			if (input.contains("touch")) {
				...
				String contentsToWrite = Arrays.toString(commands).substring(2);
				system.writeToFile(fileName, contentsToWrite);
			} else if (input.contains("read")) {
				String[] commands = input.split(" ");
				String fileName = commands[1];
				System.out.println("Contents of file : " + system.readFromFile(fileName));
			}
